...
<body class="second">
			...
			<div class="wrap_box">
				...
				<p class="tips">
					<span class="gray f_r">已有${site}帐号？请点<a class="orange bold"
						href="front/user?opr=gotoLogin">这里</a>登录
					</span>欢迎来到我们的网站，如果您是新用户，请填写下面的表单进行注册
				</p>
				${msg}
				<div class="box clearfix">
					<form action='front/user' method="post" onsubmit="return checkFormat();">
						...
						<table class="form_table f_l">
							...
							<tr>
								...
								<td><input class="gray" type="password" id="password"
									name='password' tabindex="2" /><label id="pwdmsg">填写登录密码，6-32个字符</label></td>
							</tr>
							<tr>
								<th>确认密码：</th>
								<td><input class="gray" type="password" name='repassword' id="repassword"
									tabindex="3" /><label id="repwdmsg">重复上面所填写的密码</label></td>
							</tr>
							<tr>
								<th>手机号码：</th>
								<td><input class="gray" name='phoneNum' type="text" tabindex="4" id="phoneNum"><label id="checkNum"></label></td>
							</tr>
							<tr>
								<th>电子邮箱：</th>
								<td><input class="gray" name='email' type="text" tabindex="5" id="email"><label id="checkEmail"></label></td>
							</tr>
							...
							<tr>
								<td></td>
								<td><button class="submit_reg" type="button" id="checkFormat"
								>同意一下条款，提交</button> <label></label></td>
							</tr>
						</table>
					</form>
	...
	<script>
		...
		<%--	alert("${sessionScope.captcha}")--%>
		<%--	if (this.value == "${captcha}"){--%>
		<%--		document.getElementById("checkCaptcha").innerText = "验证码输入正确";--%>
		<%--	}else {--%>
		<%--		document.getElementById("checkCaptcha").innerText = "验证码输入有误";--%>
		<%--	}--%>
		<%--})--%>
		...
		//验证邮箱格式是否正确,同时验证当前邮箱是否唯一
		document.getElementById("email").addEventListener("blur",function(){
			let emailFormat = /\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}/;
			let currentEmail = document.getElementById("email").value;
			if (emailFormat.test(currentEmail)){
				//如果邮箱能通过格式验证,则查询是否唯一
				axios.post("front/user?opr=checkEmail&email="+currentEmail).then(function(result){
					let res = result.data
					if (res.code != 200){
						document.getElementById("checkEmail").innerText = "当前邮箱已被使用";
						c1 = 0;
					}else {
						document.getElementById("checkEmail").innerText = "当前邮箱可用";
						c1 = 1;
					}
				})
			}else {
				document.getElementById("checkEmail").innerText = "邮箱格式不正确";
				c1 = 0;
			}
		})
		...
		document.getElementById("phoneNum").addEventListener("blur",function(){
			...
			if (!reg.test(document.getElementById("phoneNum").value)){
				document.getElementById("checkNum").innerText = "手机号码格式不正确"
				c2 = 0;
			}else {
				document.getElementById("checkNum").innerText = "格式正确"
				c2 = 2;
			}
		})
		...
		document.getElementById("account").addEventListener("blur",function(){
			axios.get("front/user?opr=accountIsExist&account="+this.value).then(function(result){
				if (result.data.code == 200){
					document.getElementById("namemsg").innerHTML = result.data.msg;
					c3 = 3;
				}else{
					document.getElementById("namemsg").innerHTML = "当前账号已被使用";
					c3 = 0;
				}
		...
		document.getElementById("password").addEventListener("blur",function() {
			...
			if (!reg.test(this.value)) {
				document.getElementById("pwdmsg").innerText = "密码长度为6-12位，只能包含字母和数字和下划线";
				c4 = 0;
			}else{
				document.getElementById("pwdmsg").innerText = "该密码符合要求";
				c4 = 4;
			}
		});
		...
		document.getElementById("repassword").addEventListener("blur",function(){
			...
			if (password != this.value){
				document.getElementById("repwdmsg").innerText = "两次密码必须保持一致";
				c5 = 0;
			}else {
		...
		//判断是否所有条件都能满足,都满足则可以进行注册,不满足提示
		document.getElementById("checkFormat").addEventListener("click",function(){
			...
			if(c1==1&&c2==2&c3==3&&c4==4&&c5==5){
				if (username!=""&&password!=""&&repassword!=""&&phoneNum!=""&&email!=""&&captcha!=""){
					let data = {
						"opr":"register",
						"account":username,
						"password":repassword,
					...
					axios.get("front/user",{params:data}).then(function(result){
						let res = result.data;
						if (res.code == 200){
							alert("注册成功,请前往邮箱进行激活");
							window.location.href = "front/user?opr=gotoLogin"
						}else if (res.code == 200){
							alert(res.msg);
						} else {
							alert("服务器繁忙,注册失败,请稍后重试")
						}
					})
				}else {
					alert("不能为空")
